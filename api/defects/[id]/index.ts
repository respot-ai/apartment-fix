...
import { defectUpdateSchema } from "../../_lib/validation.js";
...
export async function GET(req: Request): Promise<Response> {
  ...
  const doc = await db.collection("defects").findOne({ id }, { projection: { _id: 0 } });
...
export async function DELETE(req: Request): Promise<Response> {
  ...
  if (!id) return notFound();
  const db = await getDb();
  const result = await db.collection("defects").deleteOne({ id });
